...
public class BaseRedisUtil {
    ...
    public BaseRedisUtil(String host, int port, String password, int dbIndex) {
        this(host, port, password);
        this.dbIndex = dbIndex;
    }

    private BaseRedisUtil(String host, int port, String password) {
    ...
    public String getLockWithWait(String key, long waitTimeout, long lockTimeout) {
        ...
        try {
            ...
            // 获取锁等待时间, 超过此时间则放弃获取锁
            long timeWait = System.currentTimeMillis() + (waitTimeout * 1000);
            // 加锁后超时时间, 超过此时间则自动释放锁
            int lockSeconds = (int) lockTimeout;
            // 当前时间小于等待时间
